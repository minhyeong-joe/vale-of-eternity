...
export default function Lobby() {
	...
	useEffect(() => {
		...
		socket.on(LobbyEvents.ROOM_ADDED, (room: Room) =>
			setRooms((prev) => [...prev, room]),
		);
		socket.on(LobbyEvents.ROOM_UPDATED, (room: Room) =>
			setRooms((prev) => prev.map((r) => (r.id === room.id ? room : r))),
		);
		socket.on(LobbyEvents.ROOM_REMOVED, (roomId: string) =>
									...
									<table className="w-full">
										<thead>
											<tr className="border-b border-slate-600">
												<th className="text-left text-slate-400 text-xs font-semibold uppercase tracking-wide pb-3 pr-4">
													Room Name
												</th>
												<th className="text-left text-slate-400 text-xs font-semibold uppercase tracking-wide pb-3 pr-4">
													Host
												</th>
												<th className="text-center text-slate-400 text-xs font-semibold uppercase tracking-wide pb-3 pr-4">
													Players
												</th>
												<th className="text-center text-slate-400 text-xs font-semibold uppercase tracking-wide pb-3 pr-4">
													Pace
													...
													return (
														...
														>
															...
															<td className="py-3.5 pr-4 text-center">
																<span
																	className={`text-sm font-medium ${isFull ? "text-red-400" : "text-slate-300"}`}
																>
																	{room.currentPlayers}/{room.maxPlayers}
																</span>
															</td>
															...
															<td className="py-3.5 pr-4 text-center">
																<span className="inline-flex items-center gap-1.5 text-xs font-medium text-sky-300 bg-sky-900/40 px-2.5 py-1 rounded-full border border-sky-700/40">
																	...
																	/>
																	{room.pace.charAt(0).toUpperCase() +
																		room.pace.slice(1)}
																</span>
															</td>
