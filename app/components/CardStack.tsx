...
function TooltipPanel({ card, left, top }: TooltipPanelProps) {
	...
	return (
		...
		>
			<div
				className="bg-slate-900/98 rounded-lg shadow-2xl p-4"
				style={{ border: `1px solid ${accent}66` }}
			>
				{/* Header row: family | cost */}
				...
				{/* Card name */}
				<div
					className="text-[14px] font-semibold mb-2.5 truncate"
					style={{ color: accent }}
				>
					{card.name}
				</div>

				<div className="border-t border-slate-700/80 mb-2.5" />

				{/* One row per effect: [type icon] [description tokens] */}
				<div className="flex flex-col gap-2">
					{card.effects.map((effect, ei) => (
						<div key={ei} className="flex items-start gap-1.5">
							<div style={{ width: 34, height: 35, flexShrink: 0 }}>
								<div
									style={{
										transform: "scale(1.3)",
								...
								>
									<DescSprite name={`description-${effect.type}`} />
								</div>
							</div>
...
const TW = 262; // tooltip width + padding
const TH = 175; // tooltip height estimate

function tooltipCoords(mx: number, my: number): { left: number; top: number } {
	const left = mx + TW + 16 > window.innerWidth ? mx - TW - 10 : mx + 12;
...
export function CardImage({ card, width, height, onClick }: CardImageProps) {
...
const BACK_CARD_W = 72;
const BACK_CARD_H = 118;
const BACK_STACK_OFFSET = 12; // back cards always tight — no dynamic needed
...
export function CardStack({
	...
	const visible =
		stretch && !faceDown
			? cards
			: cards.slice(0, Math.min(cards.length, maxVisible));
	...
	if (faceDown || visible.length <= 1) {
		...
		if (spreadW <= measuredWidth) {
			...
			effectiveOffset = Math.max(
				MIN_OFFSET,
				(measuredWidth - CARD_W) / (visible.length - 1),
			);
	...
	return (
		<>
			...
			{stretch && !faceDown ? (
				...
				<div className={`flex-shrink-0 ${className}`}>{innerStack}</div>
			)}
